...
//    MHDProblem problem1(gam_hcr_1, x0_1, X_1, t0_1, T_1, h_1, tau_1, what_is_L_1);
//
//
...
//    double h2 = 0.0025;
//    double tau2 = 0.00002;
//    MHDProblem problem2(gam_hcr_1, x0_1, X_1, t0_1, T_1, h2, tau2, what_is_L_1);
...
//    double h3 = 0.0025;
//    double tau3 = 0.00002;
//    MHDProblem problem3(gam_hcr_1, x0_1, X_1, t0_1, T_1, h3, tau3, what_is_L_1);
...
//    //HLLDScheme(problem3);
    ...
    for(int k = 0; k < 5; ++k){
        ...
        problemAlfven.initStateFunc =  ([&](double x) {
            double rho_2 = 1.0;
            double u_2 = 0.;
            std::function<double(double)> xi = ([&](double x) {return x*std::cos(alpha_angle);});
            std::function<double(double)> v_2 = ([&](double x) {return 0.1 * std::sin(2*M_PI*xi(x));});
            std::function<double(double)> w_2 = ([&](double x) {return 0.1 * std::cos(2*M_PI*xi(x));});
            double p_2 = 0.1;
            double Bx_2 = std::sqrt(4*M_PI);
            std::function<double(double)> By_2 = ([&](double x) {return 0.1 *std::sqrt(4*M_PI)*std::sin(2*M_PI*xi(x));});
            std::function<double(double)> Bz_2 = ([&](double x) {return 0.1 * std::sqrt(4*M_PI)*std::cos(2*M_PI*xi(x));});
